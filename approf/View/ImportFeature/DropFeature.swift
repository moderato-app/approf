    ...
    Reduce { state, action in
      ...
      case let .onDropEnds(urls):
        ...
        if filePaths.isEmpty {
          ...
          let basic = PProfBasic(uuid: uuid(), filePaths: filePaths, presentation: .dft)
          return .run { send in
          ...
          state.destination = .uth(UnderTheHood.State(basic: Shared(PProfBasic(uuid: uuid(), filePaths: filePaths, presentation: .diff))))
          return .none
        } else {
          state.destination = .uth(UnderTheHood.State(basic: Shared(PProfBasic(uuid: uuid(), filePaths: filePaths, presentation: .acc))))
          return .none
        }
        ...
        if case .dft = uthFeature.basic.presentation {
          ...
          return .run { send in
            for fp in filePaths.reversed() {
              ...
              let basic = PProfBasic(uuid: uuid(), filePaths: [fp], presentation: .dft)
              await send(.delegate(.addNewBasic(basic)), animation: .default)
          ...
          let basic = PProfBasic(uuid: uuid(), filePaths: uthFeature.basic.filePaths, presentation: uthFeature.basic.presentation)
          return .run { send in
            try await clock.sleep(for: .seconds(0.03))
            await send(.delegate(.addNewBasic(basic)), animation: .default)
            try await clock.sleep(for: .seconds(0.03))
