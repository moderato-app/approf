...
struct PProfBasic: Equatable, Identifiable, Codable {
  ...
  init(uuid: UUID, filePaths: [String], presentation: PProfPresentation = .dft, httpDetectLog: [HTTPResult] = [], terminalOutput: [TerminalRecord] = [], finalCommandArgs: [CommandLine.CommandArg] = []) {
    self.id = uuid
    self.name = ""
  ...
  init(uuid: UUID, urls: [URL], presentation: PProfPresentation = .dft) {
    let filePaths = urls.map { $0.path(percentEncoded: false) }
    self.init(uuid: uuid, filePaths: filePaths, presentation: presentation)
  }
